...
test('ownership task package seeds contain base + external care packages for every age group', () => {
  ...
  for (const ageGroup of ageGroups) {
    ...
    assert.equal(categories.betreuung_entwicklung.length, 13);
    assert.equal(categories.gesundheit.length, 9);
    assert.equal(categories.babyalltag_pflege.length, 13);
    assert.equal(categories.haushalt_einkaeufe_vorraete.length, 13);
  }
});
...
test('seed includes the final fixed titles for every age range', () => {
  ...
  assert.equal(
    ownershipTaskPackageSeedByAgeGroup['0_1'].betreuung_entwicklung.find((entry) => entry.title.de === 'Kita-Kommunikation steuern')?.filterTags?.[0],
    'externalCare:kita',
  );
});
